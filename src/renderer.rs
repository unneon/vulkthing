...
impl Renderer {
    ...
    unsafe fn record_render_pass(
        ...
        if voxel_meshlet_count > 0 {
            ...
            begin_label(buf, "Debug voxel chunk draw", [133, 0, 0], &self.dev);
            self.bind_graphics_pipeline(buf, self.pipelines.debug_voxel_chunk);
            self.bind_descriptor_set(buf, self.pipeline_layouts.debug_voxel_chunk);
            unsafe { self.dev.mesh_ext.cmd_draw_mesh_tasks(buf, 1, 1, 1) };
            end_label(buf, &self.dev);
        }
