bool screen_aabb_projection(vec3 min_coords, vec3 max_coords, float depth_near, mat4 projection_view, out vec4 screen_aabb) {
    ...
    vec4 p6 = p4 + sy;
    vec4 p7 = p6 + sz;
    if (min(min(min(p0.w, p1.w), min(p2.w, p3.w)), min(min(p4.w, p5.w), min(p6.w, p7.w))) < depth_near) {
        return false;
    }
    screen_aabb.xy = min(
