...
vec3 world_space_from_depth(float depth, vec4 frag_coord) {
    ...
    vec2 window_space = 2 * frag_coord.xy / global.camera.resolution - 1;
    ...
    vec4 unnormalized_view_space = global.camera.inverse_projection_matrix * normalized_clip_space;
    vec4 view_space = vec4(unnormalized_view_space.xyz / unnormalized_view_space.w, 1);
    vec4 world_space = global.camera.inverse_view_matrix * view_space;
